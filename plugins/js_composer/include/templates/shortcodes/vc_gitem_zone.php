...
if ( strlen( $link ) > 0 && 'none' !== $link ) {
	...
	if ( 'custom' === $link && ! empty( $url ) ) {
		...
		*/
		...
		$image_block .= '<a href="{{ post_image_url }}" title="{{ post_title }}" data-slug="{{ slug }}"' . $this->prettyphoto_rel . ' data-vc-gitem-zone="prettyphotoLink" class="vc_gitem-link prettyphoto vc-zone-link vc-prettyphoto-link"></a>';
	}
